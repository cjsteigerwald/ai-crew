...
# noflockpath: a bin dir holding every external command ai-crew.sh uses EXCEPT
# flock, echoed on stdout. A script run with this as its PATH sees what stock
# macOS really has — no flock(1) at all — so the mkdir lock takes over.
noflockpath() {
  local d="$F/noflock-bin" c
  mkdir -p "$d"
  for c in bash env git jq sha256sum shasum cmp sed date mkdir rmdir rm mv cat dirname basename tail grep awk sort tr cut ls wc mktemp chmod find sleep readlink realpath; do
    [ -x "$(command -v "$c" 2>/dev/null)" ] && ln -sf "$(command -v "$c")" "$d/$c"
  done
  echo "$d"
}

# held <lockfile> <cmd...>: run <cmd> with the update lock HELD, the way THIS
# host holds it. `flock <file> <cmd>` cannot be the harness's way of holding it:
# stock macOS ships no flock(1), so there the wrapper itself is what fails, with
# rc 127, and the test proves nothing about the script.
held() {
  local lock=$1; shift
...
# held_mkdir <lockfile> <cmd...>: the same, forced onto the mkdir lock whatever
# the host happens to have — set up directly, with this harness's own pid as the
# (live) holder, and the command run with flock off its PATH so the script takes
# the branch it would take on macOS. That is the branch macOS runs in
# production, so it is exercised on every platform rather than only where flock
# is missing.
held_mkdir() {
  local lock=$1 rc; shift
  mkdir -p "$lock.d"; echo $$ >"$lock.d/pid"
  PATH="$(noflockpath)" "$@"
  rc=$?
  rm -rf "$lock.d"
  return $rc
...
mkfix; S gate >/dev/null; S snapshot >/dev/null
t "update refuses while lock held" 1 "another update in progress" -- held "$F/snap/update.lock" "$SCRIPT" update
...
EOF
mkdir -p "$F/snap"
t "reconcile refuses while the update lock is held" 1 "in progress" -- held "$F/snap/update.lock" "$SCRIPT" reconcile
t "reconcile --dry-run does not need the lock" 0 "^reconcile: DRY RUN" -- held "$F/snap/update.lock" "$SCRIPT" reconcile --dry-run
# And again on the mkdir lock specifically, which is the only lock a host
# without flock(1) has — reconcile's refusal was otherwise proven for the flock
# branch alone.
t "reconcile refuses while the mkdir lock is held" 1 "in progress" -- held_mkdir "$F/snap/update.lock" "$SCRIPT" reconcile
t "reconcile --dry-run does not need the mkdir lock either" 0 "^reconcile: DRY RUN" -- held_mkdir "$F/snap/update.lock" "$SCRIPT" reconcile --dry-run
...
# The same manifest on a BSD/macOS userland. GNU realpath resolves a path whose
# LAST component does not exist ("all but the last component must exist" is the
# documented default); BSD realpath(1) wraps realpath(3), which returns ENOENT
# for any missing component, last one included, and offers no -m. A stale
# registration naming a hook the plugin no longer ships on disk is EXACTLY what
# reconcile exists to remove, so canon_path canonicalizes the parent directory
# itself rather than trusting the host's realpath to accept a missing tail.
# Without that, removal would silently stop on every BSD box — and a Linux-only
# suite would never see it.
mkfix
BSDCANON="$F/bsdcanon"; mkdir -p "$BSDCANON"
cat >"$BSDCANON/realpath" <<EOF
#!/usr/bin/env bash
# BSD realpath(1): the WHOLE path must exist. No -m, no -e.
p=\$1; [ "\$p" = "--" ] && p=\$2
[ -e "\$p" ] || { echo "realpath: \$p: No such file or directory" >&2; exit 1; }
exec $(command -v realpath) -- "\$p"
EOF
cat >"$BSDCANON/readlink" <<EOF
#!/usr/bin/env bash
# BSD readlink -f resolves through realpath(3) as well and refuses the same
# paths. Every other invocation (the plain link read) passes straight through.
if [ "\$1" = "-f" ]; then
  p=\$2; [ "\$p" = "--" ] && p=\$3
  [ -e "\$p" ] || exit 1
fi
...
EOF
chmod +x "$BSDCANON/realpath" "$BSDCANON/readlink"
mkdir -p "$(ALPHA_PATH)/hooks" "$F/outside/hooks"
cat >"$(ALPHA_PATH)/hooks/hooks.json" <<'EOF'
{"hooks":{"PreToolUse":[{"matcher":"Agent|Task","hooks":[{"type":"command","command":"python3 \"${CLAUDE_PLUGIN_ROOT}/hooks/lane-model-gate.py\""}]}]}}
EOF
ln -s "$F/outside/hooks" "$(ALPHA_PATH)/linked"
# Three registrations, none of them backed by a file: the plugin's own hook was
# renamed away, so only proof 1 can decide any of them.
jq -n --arg r "$(ALPHA_PATH)/hooks/lane-model-gate.py" --arg l "$(ALPHA_PATH)/linked/lane-model-gate.py" \
      --arg d "$(ALPHA_PATH)/no-such-dir/lane-model-gate.py" '{hooks: {PreToolUse: [
  {matcher: "root",   hooks: [{type: "command", command: ("python3 " + $r)}]},
  {matcher: "linked", hooks: [{type: "command", command: ("python3 " + $l)}]},
  {matcher: "deep",   hooks: [{type: "command", command: ("python3 " + $d)}]}]}}' | mksettings
t "BSD-style realpath: the shim refuses a missing last component" 1 "No such file" -- env PATH="$BSDCANON:$PATH" bash -c 'realpath -- "$1"' _ "$(ALPHA_PATH)/hooks/lane-model-gate.py"
t "BSD-style realpath: the shim still resolves a path that exists" 0 "hooks$" -- env PATH="$BSDCANON:$PATH" bash -c 'realpath -- "$1"' _ "$(ALPHA_PATH)/hooks"
t "BSD-style realpath: a vanished hook under the plugin root is still removed" 0 "removed legacy hook PreToolUse/lane-model-gate.py" -- env PATH="$BSDCANON:$PATH" "$SCRIPT" reconcile
t "BSD-style realpath: that registration is gone" 0 "" -- jq -e '[.hooks.PreToolUse[] | select(.matcher == "root")] | length == 0' "$AI_CREW_SETTINGS"
# The security property the resolved-only rule bought must survive the fallback:
# canonicalizing the PARENT still resolves the symlinked component, so a path
# that leaves the plugin directory is not proven by its spelling.
t "BSD-style realpath: a vanished hook reached through a symlink OUT is KEPT" 0 "" -- jq -e '[.hooks.PreToolUse[] | select(.matcher == "linked")] | length == 1' "$AI_CREW_SETTINGS"
t "BSD-style realpath: a missing DIRECTORY component is still unresolvable" 0 "" -- jq -e '[.hooks.PreToolUse[] | select(.matcher == "deep")] | length == 1' "$AI_CREW_SETTINGS"

# The same two, on the host's own userland: GNU resolves the missing tail
# itself, and must reach the identical verdict on both.
mkfix
mkdir -p "$(ALPHA_PATH)/hooks" "$F/outside/hooks"
cat >"$(ALPHA_PATH)/hooks/hooks.json" <<'EOF'
{"hooks":{"PreToolUse":[{"matcher":"Agent|Task","hooks":[{"type":"command","command":"python3 \"${CLAUDE_PLUGIN_ROOT}/hooks/lane-model-gate.py\""}]}]}}
EOF
ln -s "$F/outside/hooks" "$(ALPHA_PATH)/linked"
jq -n --arg r "$(ALPHA_PATH)/hooks/lane-model-gate.py" --arg l "$(ALPHA_PATH)/linked/lane-model-gate.py" '{hooks: {PreToolUse: [
  {matcher: "root",   hooks: [{type: "command", command: ("python3 " + $r)}]},
  {matcher: "linked", hooks: [{type: "command", command: ("python3 " + $l)}]}]}}' | mksettings
t "host realpath: a vanished hook under the plugin root is removed" 0 "removed legacy hook PreToolUse/lane-model-gate.py" -- S reconcile
t "host realpath: a vanished hook reached through a symlink OUT is KEPT" 0 "" -- jq -e '[.hooks.PreToolUse[] | select(.matcher == "linked")] | length == 1' "$AI_CREW_SETTINGS"
