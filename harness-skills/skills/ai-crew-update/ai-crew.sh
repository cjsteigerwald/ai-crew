...
# canon_one <path>: canonicalize with whatever this host has, or nothing and
# exit 1. Used only by canon_path, which adds the portability the two tools
# differ on.
canon_one() {
  local r
...
# this box, or a DIRECTORY component of the path does not exist).
#
...
#
# The LAST component need not exist. GNU realpath resolves such a path by
# default ("all but the last component must exist" — coreutils manual; -e is
# what demands the last one too), but BSD/macOS realpath(1) is a wrapper around
# realpath(3), which returns ENOENT when any component including the last is
# missing, and offers no -m. A registration naming a hook the plugin no longer
# ships is EXACTLY the case this tool exists to clean up, so leaving it to the
# host's realpath would silently stop removing legacy hooks on every BSD box.
# The GNU semantics are therefore reproduced here rather than relied upon: the
# PARENT DIRECTORY is canonicalized — it must exist, so every symlinked
# component is still resolved and a path that leaves the plugin directory
# through a link still resolves outside it — and the basename is appended
# literally.
#
# A last component that DOES exist in some form the host could not resolve (a
# dangling symlink, say) is not second-guessed: it returns 1, and unproven
# means kept.
canon_path() {
  local p=$1 base parent r
  canon_one "$p" && return 0
  [ ! -e "$p" ] && [ ! -L "$p" ] || return 1
  while [ "${p%/}" != "$p" ]; do
    p=${p%/}
    [ -n "$p" ] || return 1
...
prov_of() {
  ...
  #    DIRECTORY component that does not exist — this proof is not available AT
  #    ALL and the entry falls through to the content hash and the migration
  #    record.
