   ...
   - **`settings.json` — hooks are REMOVED, never added.** It reads the
     ...
     - **Every referencing token has PROVENANCE** — positive proof that the
       ...
       1. **It resolves inside the plugin's own installed directory** — the
          ...
          itself need not still exist — that is the ordinary shape of a stale
          registration, and it resolves the same way on GNU and BSD userlands
          because the tool canonicalizes the parent DIRECTORY itself rather
          than trusting the host's `realpath` to accept a missing last
          component. But the path must be resolvable — if it is not (a
          directory component that does not exist, a last component the host
          cannot resolve, or a host with neither `realpath` nor `readlink -f`),
          this proof is simply unavailable and the entry falls through to 2 and
          3, which keeps it unless one of them holds.
